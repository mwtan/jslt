...
/**
 * For now contains all the various function implementations. Should probably be
 * broken up into separate files and use annotations to capture a lot of this
 * information instead.
 */
public class BuiltinFunctions {
	...
	static {
		...
		functions.put("abbreviate", new BuiltinFunctions.Abbreviate());
		functions.put("capitalize", new BuiltinFunctions.Capitalize());
		functions.put("center", new BuiltinFunctions.Center());
		functions.put("defaultIfBlank", new BuiltinFunctions.DefaultIfBlank());
		...
		functions.put("toLowerCase", new BuiltinFunctions.Lowercase());
		functions.put("uppercase", new BuiltinFunctions.Uppercase());
		functions.put("toUpperCase", new BuiltinFunctions.Uppercase());
		...
		functions.put("substring", new BuiltinFunctions.SubString());
	...
	public static Map<String, Macro> macros = new HashMap<String, Macro>();
	static {
	...
	// ===== ABBREVIATE
	public static class Abbreviate extends AbstractFunction {
		public Abbreviate() {
			super("abbreviate", 2, 2);
		}
		public JsonNode call(JsonNode input, JsonNode[] arguments) {
			String string = NodeUtils.toString(arguments[0], false);
			int maxwidth = Integer.parseInt(NodeUtils.toString(arguments[1], true));
			return new TextNode(StringUtils.abbreviate(string, maxwidth));
		}
	}
	
	// ===== CAPITALIZE
	public static class Capitalize extends AbstractFunction {
		public Capitalize() {
			super("capitalize", 1, 1);
		}
		public JsonNode call(JsonNode input, JsonNode[] arguments) {
			String string = NodeUtils.toString(arguments[0], false);
			return new TextNode(StringUtils.capitalize(string));
		}
	}
	
	// ===== CENTER
	public static class Center extends AbstractFunction {
		public Center() {
			super("center", 2, 2);
		}
		public JsonNode call(JsonNode input, JsonNode[] arguments) {
			String string = NodeUtils.toString(arguments[0], false);
			int size = Integer.parseInt(NodeUtils.toString(arguments[1], true));
			return new TextNode(StringUtils.center(string, size));
		}
	}

	// ===== DEFAULTIFBLANK
	public static class DefaultIfBlank extends AbstractFunction {
		public DefaultIfBlank() {
			super("defaultIfBlank", 2, 2);
		}
		public JsonNode call(JsonNode input, JsonNode[] arguments) {
			String string = NodeUtils.toString(arguments[0], false);
			String defstr = NodeUtils.toString(arguments[1], true);
			return new TextNode(StringUtils.defaultIfBlank(string, defstr));
		}
	}
	...
	public static class SubString extends AbstractFunction {
		...
		public JsonNode call(JsonNode input, JsonNode[] arguments) {
			...
			int start = Integer.parseInt(NodeUtils.toString(arguments[1], false));
			int end = Integer.parseInt(NodeUtils.toString(arguments[2], false));
			return new TextNode(string.substring(start, end));
		}
	}
