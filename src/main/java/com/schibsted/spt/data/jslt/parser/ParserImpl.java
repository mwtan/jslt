...
public class ParserImpl {
  ...
  private static ExpressionImpl compile(ParseContext ctx, SimpleNode root) {
	...
	else if (expr.id == JsltParserTreeConstants.JJTEXPRLIST) {
		top = node2exprlist(ctx, expr);
	}
	...
	private static ExpressionNode node2exprlist(ParseContext ctx, SimpleNode eparentnode) {
		if (eparentnode.id != JsltParserTreeConstants.JJTEXPRLIST) {
			throw new JsltException("INTERNAL ERROR: Wrong type of node: " + eparentnode);
		}
		ExpressionList elist = new ExpressionList(makeLocation(ctx, eparentnode));
		// figure out how many expressions there are
	    int ecount = countChildren(eparentnode, JsltParserTreeConstants.JJTEXPR);
		for (int ix = 0; ix < ecount; ix++) {
			SimpleNode exprnode = (SimpleNode) eparentnode.jjtGetChild(ix);
			ExpressionNode first = node2andexpr(ctx, getChild(exprnode, 0));
		    if (exprnode.jjtGetNumChildren() == 1) {
		    	elist.add(first);
		    }
		    else {
		    	ExpressionNode second = node2expr(ctx, getChild(exprnode, 1));
		    	elist.add(new OrOperator(first, second, makeLocation(ctx, exprnode)));
		    }
		}
		return elist;
	}
  ...
  private static ExpressionNode node2baseExpr(ParseContext ctx, SimpleNode node) {
    ...
    if (token.kind == JsltParserConstants.LBRACKET ||
        ...
        token.kind == JsltParserConstants.FOR ||
    ...
    if (kind == JsltParserConstants.NULL)
      ...
      return new LiteralExpression(BooleanNode.FALSE, loc);
             ...
             kind == JsltParserConstants.SLASH ||
      ...
      if (maybeelse.jjtGetFirstToken().kind == JsltParserConstants.ELSE) {
        ...
        theelse = node2exprlist(ctx, elseexpr);
        elselets = buildLets(ctx, maybeelse);
      }

      LetExpression[] thenlets = buildLets(ctx, node);

      return new IfExpression(
        node2expr(ctx, (SimpleNode) node.jjtGetChild(0)),
        thenlets,
        node2exprlist(ctx, (SimpleNode) node.jjtGetChild(thenlets.length + 1)),
        elselets,
        theelse,
        loc
      );
    ...
    } else if (kind == JsltParserConstants.FOR) {
        LetExpression[] lets = buildLets(ctx, node);
        ExpressionNode loopExpr = node2exprlist(ctx, getLastChild(node));
        return new ForTextExpression(node2expr(ctx, (SimpleNode) node.jjtGetChild(0)), 
        		lets, loopExpr, null, makeLocation(ctx, node));
  ...
  private static ExpressionNode chainable2Expr(ParseContext ctx, SimpleNode node) {
    ...
    } else if (kind == JsltParserConstants.DOT || kind == JsltParserConstants.SLASH) {
      token = token.next;
  ...
  private static ExpressionNode buildDotChain(ParseContext ctx,
	...
	private static ExpressionNode buildChainLink(ParseContext ctx, 
												 ...
												 ExpressionNode parent) {
		Token token = node.jjtGetFirstToken();
		if (token.kind == JsltParserConstants.DOT || token.kind == JsltParserConstants.SLASH) {
			// it's a dotkey
